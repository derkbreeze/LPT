...
class MOT16Sequence(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        sample['gt'] = data['gt']
        sample['vis'] = data['vis']
        sample['det'] = data['det'] #added by S.L
        ...
        #added by S.L
        cropped_det_imgs = {}
        for det_id, box in sample['det'].items():
            box_crop = box.astype(np.int).clip(0, None)
            crop = img[:, box_crop[1]:box_crop[3], box_crop[0]:box_crop[2]]
            cropped_det_imgs[det_id] = crop
        sample['cropped_det_imgs'] = cropped_det_imgs
        #added by S.L
    ...
    def _sequence(self):
        ...
        if seq_name in self._train_folders:
            seq_path = os.path.join(self._mot_dir, 'train', seq_name)
        else:
            seq_path = os.path.join(self._mot_dir, 'test', seq_name)
        config_file = os.path.join(seq_path, 'seqinfo.ini')
        ...
        img_dir = os.path.join(seq_path, img_dir)
        gt_file = os.path.join(seq_path, 'gt', 'gt.txt')

        #det_file = osp.join(seq_path, 'det', 'det.txt') #S.L added
        #det_file = osp.join(seq_path, 'det', 'det_DPM.txt') #Tracktor filtered DPM detection
        #det_file = osp.join(seq_path, 'det', 'det_FRCNN.txt') #Tracktor filtered FRCNN detection
        det_file = os.path.join(seq_path, 'det', 'det_SDP.txt') #Tracktor filtered SDP detection
        seg_dir = os.path.join(seq_path, 'seg_ins')
        ...
        for i in range(1, seqLength+1): #matlab annotation starts at frame 1
            boxes[i] = {}
            det_boxes[i] = {}  #S.L added
            visibility[i] = {}
        ...
        if os.path.exists(gt_file):
            with open(gt_file, "r") as inf:
                ...
                for row in reader:
                    # class person, certainity 1, visibility >= 0.1
        ...
        #S.L added to handle testing cases where detections can be arbitrary
        if os.path.exists(det_file):
            with open(det_file, "r") as inf:
                dets = np.loadtxt(inf, delimiter=',')
                dets = np.concatenate([dets, np.arange(dets.shape[0])[:,None]], axis=1)
                for row in range(dets.shape[0]):
                    x1 = int(dets[row, 2]) - 1
                    y1 = int(dets[row, 3]) - 1
                    # This - 1 accounts for the width (width of 1 x1=x2)
                    x2 = x1 + int(dets[row, 4]) - 1
                    y2 = y1 + int(dets[row, 5]) - 1
                    score = dets[row, 6]
                    bb = np.array([x1,y1,x2,y2,score], dtype=np.float32)
                    det_boxes[int(dets[row, 0])][int(dets[row, 10])] = bb

        if self._load_seg:
            if os.path.exists(seg_dir):
                for seg_file in listdir_nohidden(seg_dir):
                    frame_id = int(seg_file.split('.')[0])
                    seg_img = Image.open(os.path.join(seg_dir, seg_file))
                    seg_imgs[frame_id] = seg_img

        for i in range(1, seqLength + 1):
            img_path = os.path.join(img_dir, '{:06d}.jpg'.format(i))
            datum = {'gt': boxes[i], 'det': det_boxes[i], 
                     'im_path': img_path, 'vis': visibility[i]}#S.L added
    ...
    def write_results(self, all_tracks, output_dir):
        ...
        file = os.path.join(output_dir, 'MOT16-'+self._seq_name[6:8]+'.txt')
